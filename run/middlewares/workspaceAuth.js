...
module.exports = async (req, res, next) => {
  ...
  // Prefer firebaseUserId from header over query param (avoids URL leakage)
  const headerFirebaseUserId = req.headers["x-firebase-user-id"];
  if (headerFirebaseUserId && !data.firebaseUserId) {
    data.firebaseUserId = headerFirebaseUserId;
  }

  try {
    if (!data.workspace) throw new Error("Missing parameter");
    ...
    if (authorizationHeader) {
      ...
      if (headerSplit.length > 1) {
        ...
        if (!user) throw new Error(`Invalid firebaseUserId`);
    ...
    const workspace = await db.getWorkspaceByName(
      data.firebaseUserId || firebaseUser.user_id,
      data.workspace,
    );
    ...
    logger.error(error.message, {
      location: "middleware.workspaceAuth",
      error: error,
      data: data,
    });
