  ...
  try {
    ...
    if (authorizationHeader) {
      ...
      if (headerSplit.length > 1) {
        ...
        const user = await db.getUser(jwtData.firebaseUserId, [
          "apiKey",
          "stripeCustomerId",
        ]);
      ...
      if (!firebaseUser) throw new Error("You must be signed in to do this.");
      req.body.data.user = await db.getUser(firebaseUser.user_id, [
        "apiKey",
        "stripeCustomerId",
      ]);
      ...
      next();
    } else throw new Error("You must be signed in to do this.");
  } catch (error) {
